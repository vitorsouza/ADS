...
public abstract class BaseJPADAO<T extends PersistentObject> implements BaseDAO<T> {
	 ...
	 */
	protected void applyOrdering(CriteriaBuilder cb, Root<T> root, CriteriaQuery cq) {
	...
	protected T executeSingleResultQuery(CriteriaQuery cq, Object ... params) throws PersistentObjectNotFoundException, MultiplePersistentObjectsFoundException {
		// Looks for a single result. Throws a checked exception if the entity is not found or in case of multiple results.
		try {
			Object result = getEntityManager().createQuery(cq).getSingleResult();
			return (T)result;
		}
	...
	public long retrieveFilteredCount(Filter<?> filter, String value) {
		...
		EntityManager em = getEntityManager();
		CriteriaQuery cq = buildFilteredCriteriaQuery(filter, value, true);
		Query q = em.createQuery(cq);
	...
	public List<T> retrieveWithFilter(Filter<?> filter, String value) {
		...
		EntityManager em = getEntityManager();
		CriteriaQuery cq = buildFilteredCriteriaQuery(filter, value, false);
		return em.createQuery(cq).getResultList();
	}
	 ...
	 * @param toCount
	 *		Flag that indicates if we're building the criteria query for a count (<code>true</code>) or for a normal retrieve (<code>false</code>).
	 *
	 * @return The criteria query object that can be used to produce the query.
	 */
	private CriteriaQuery buildFilteredCriteriaQuery(Filter<?> filter, String value, boolean toCount) {
		// Using the entity manager, create a criteria query to retrieve objects of the domain class.
		EntityManager em = getEntityManager();
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery cq = cb.createQuery();
		...
		// If the query serves a count of objects, select the count. Otherwise, select the domain class objects.
		if (toCount)
		...
		switch (filter.getType()) {
			...
			case REVERSE_MULTIPLE_CHOICE:
				...
				//		select subDep.id from <sub-class> sub inner join sub.<sub-field-name> subDep where sub.id = <value-converted-as-id>
				// )
		...
		// Returns the criteria query object.
		return cq;
	}
	...
	/**
	 * Finds the final path that has to be compared in the criterion. In case of simple fields (e.g. name, representing a String), this is
	 * straightforward. In case of fields that require navigation (e.g. address.city.name), this method performs the necessary joins and
	 * returns the Path object representing the correct property (in the example, name) to be compared.
	 *
	 * @param root
	 ...
	 * @return
	 *		The Path object representing the property that should be compared.
	 */
