 ...
 * 
 * This class integrates with the CrudServiceLocal (EJB3) interface and must deal with a subclass of PersistentObject, which
 * is specified as a generic parameter, along with the class of its ID. It provides basic controller functions,
 ...
 * @version 1.1
 */
public abstract class CrudAction<T extends PersistentObject> extends JSFAction implements Serializable {
	...
	/** The logger. */
	private static Logger logger = Logger.getLogger(CrudAction.class.getCanonicalName());
	...
	/**
	 * Informs to other methods what is the maximum number of entities to be displayed in a listing page. This method
	 * exists so it can be overridden by subclasses if desired.
	 * 
	 * @return The maximum number of entities to be displayed in the page.
	 */
	protected int getMaxEntitiesPerPage() {
	 ...
	 * overridden by subclasses if they don't follow the standard naming convention for Crud Actions, which is:
	...
	/**
	 * Obtains the name of the form field in the HTML page, given the name of the property it represents. It defaults to "form:" +
	 * propertyName, but it can be overridden in case of need.
	 *
	 * @param propertyName
	...
	public void goFirst() {
		...
		// Always counts the entities in this method, as it can be called via AJAX from the pages. This also sets the last entity index.
		count();
	...
	public void goLast() {
		...
		if (entityCount == 0)
			...
			// Calculates how many entities there are in the last page (the remainder of dividing the count by the max entities in a page).
			int remainder = ((int)entityCount % MAX_ENTITIES_PER_PAGE);

			// Check if the remainder is zero, in which case the last page is full. Otherwise, the remainder is the number of entities in
	...
	public void trash(Long id) {
		...
		selectedEntity = getCrudService().retrieve(id);
		logger.log(Level.INFO, "Adding " + selectedEntity + " (id " + id + ") to the trash can for future deletion.");
	...
	public String retrieve(Long id) {
		...
		// Retrieves the existing entity that was selected.
